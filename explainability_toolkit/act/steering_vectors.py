...
def find_steering_vecs(base, target, model, layer, batch_size = 16, pos=-1): 
    '''
    We want to find the steering vector from base_toks to target_toks (we do target_toks - base_toks)
    Inputs: 
        :param model: the model to use
        :param base: base [seq_len_base]
        :param target: target [seq_len_target]
    Output: 
    ...
    base_toks = model.to_tokens(base).to(DEVICE)
    target_toks = model.to_tokens(target).to(DEVICE)
    
    for sign, toks in zip([-1, 1], [base_toks, target_toks]):
        for i in tqdm(range(0, len(toks), batch_size)):
            # add the hook
            h = model.blocks[layer].register_forward_pre_hook(get_activation(layer))
            # get the activations
            model(base_toks[i:min(i + batch_size, len(toks))].to(device))
            out = activations[layer]
            # average over the batch_size, take last token 
            if i == 0: 
                if pos == None:
                    steering_vecs = sign * torch.mean(out.detach().cpu(), dim=0)/len(toks) # [seq_len, hidden_size]
                else: 
                    steering_vecs = sign * torch.mean(out[:,pos,:].detach().cpu(), dim=0)/len(toks) # [hidden_size]
            else: 
                if pos == None:
                    steering_vecs += sign * torch.mean(out.detach().cpu(), dim=0)/len(toks)
                else: 
                    steering_vecs += sign * torch.mean(out[:,pos,:].detach().cpu(), dim=0)/len(toks)
            # remove the hook
            h.remove()
...
def optimise_steering_vecs(model, toks_A, toks_B, hidden_dim, layer, type='full', batch_size=6, num_epochs=10): 
    ...
    h_o = model.blocks[layer].register_forward_pre_hook(sv_hook)
    if type=='reft':
    ...
    if steering_vec is not None: 
        ...
        for i in range(len(model.blocks)):
            if layer is None: # append to each layer
                handles.append(model.blocks[i].register_forward_pre_hook(modify_activation()))
